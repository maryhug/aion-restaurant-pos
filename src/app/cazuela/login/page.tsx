...
const DEMO_CREDENTIALS = {
  cliente: { email: "cliente4@gmail.com", password: "Cliente1234!" },
  staff: { email: "staff1@lacazuela.com", password: "Staff1234!" },
  admin: { email: "admin@lacazuela.com", password: "lacazuela2024!" },
} as const;

export default function CazuelaLoginPage() {
  ...
  async function doLogin(loginEmail: string, loginPassword: string) {
    ...
    try {
      ...
      if (!res.ok) {
        setErr((data as ApiError).error ?? "No se pudo iniciar sesión.");
        return;
      }
  ...
  return (
    <div
      ...
      style={{ background: t.colors.pageBgBeige, color: t.colors.text }}
    >
      <p
      ...
      <p className="mt-1 text-sm" style={{ color: t.colors.muted }}>
        Sistema POS Inteligente
      </p>
      <div className="mt-8 w-full max-w-sm rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
        <h1 className="text-center text-lg font-extrabold">Bienvenido</h1>
        <p className="text-center text-xs" style={{ color: t.colors.muted }}>
        ...
        <div
          className="my-4 flex items-center gap-2 text-xs"
          style={{ color: t.colors.muted }}
        >
            ...
            <button
              ...
              style={
                demoActive === role
                  ...
                  : { borderColor: "#e7e5e4", color: t.colors.text }
              }
            >
              {demoActive === role
                ? "Entrando…"
